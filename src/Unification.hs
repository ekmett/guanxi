...
class (MonadPlus m, EqVar v) => MonadUnify v m | m -> v where
  newVar :: m (v f)
...
unifyVar :: (MonadUnify v m, Unified f) => v f -> Free f (v f) -> m (Free f (v f))
...
unify :: (MonadUnify v m, Unified f) => Free f (v f) -> Free f (v f) -> m (Free f (v f))
