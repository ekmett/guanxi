...
newLogState :: Monoid a => Bool -> LogState a
...
watchNew :: Monoid a => LogState a -> (Int, LogState a)
...
watchOld :: Monoid a => LogState a -> (Int, LogState a)
...
data Log u a where
  Log :: Monoid a => { getLog :: Ref u (LogState a) } -> Log u a

log :: HasRefEnv s u => Log u a -> Lens' s (LogState a)
log = ref . getLog

newLog
  :: (MonadState s m, MonadKey m, HasRefEnv s (KeyState m), Monoid a)
  => Bool
  -> m (Log (KeyState m) a)
newLog = fmap Log . newRef . newLogState

-- | Get a snapshot of the # of cursors outstanding
cursors :: (MonadState s m, HasRefEnv s u, Monoid a) => Log u a -> m Int
cursors l = uses (log l) $ \(LogState t _ c _) -> refCount (measure t) + c 

record :: (MonadState s m, HasRefEnv s u, Semigroup a) => Log u a -> a -> m ()
...
newCursor, oldCursor :: (MonadState s m, HasRefEnv s (KeyState m), MonadKey m, Monoid a) => Log (KeyState m) a -> m (Cursor (KeyState m) a)
newCursor l = Cursor l <$> do
  i <- log l %%= watchNew 
  newRef i

oldCursor l = Cursor l <$> do
  i <- log l %%= watchOld
  newRef i
...
advance :: (MonadState s m, HasRefEnv s u) => Cursor u a -> m a
advance (Cursor rl@Log{} ri) = do
  i <- readRef ri 
  join $ log rl %%= \ls@(LogState t v c m) -> if
    | i >= v -> case m of
      Nothing -> (pure mempty, ls)
      Just a
        | c == 1 -> case viewr t of
          t' F.:> LogEntry ov oc b -> (pure a , LogState (t' F.|> LogEntry ov oc (mappend b a)) v 1 Nothing)
          EmptyR -> (pure a, LogState t v c Nothing) -- we're the only one listening
        | !v' <- v + 1 -> (a <$ writeRef ri v', LogState (t F.|> LogEntry v (c-1) a) v' 1 Nothing)
    | otherwise -> case F.split (\(LogEntry j _ _) -> j >= i) t of
      (l,r) -> case viewr l of
        EmptyR -> error "advance: missing cursor!"
        l' F.:> LogEntry j c' a
          | (ls', k) <- watchNew (LogState (nl >< r) v c m) -> ((a <> contents (measure r) <> Foldable.fold m) <$ writeRef ri ls', k) where
          nl | c' > 1 = l' F.|> LogEntry j (c'-1) a
             | otherwise = case viewr l' of
               l'' F.:> LogEntry k c'' b -> l'' F.|> LogEntry k c'' (mappend b a)
               EmptyR -> mempty
